...
# This script does not produce wbq's and percentiles for all users. That is  
# because conversion tables for English were not available, and instead 
# the tables for the Dutch Peabody test were used. So some users have raw 
# scores for which no wbq or percentile is defined in the table.

# The script currently uses the manual calculation of the total errors as to
# calculate the raw score, because many users in the data do not have a 
# summary.
...
experiment_abr <- "bq4english"
server_url <- "http://frinexproduction.mpi.nl"
admin_url <- paste(server_url, "/", experiment_abr, "-admin", sep = "")

# Load tables for score conversions
peabody_dat = read.csv("peabody_english/Peabody.scores.csv", header = TRUE)
peabody_percent = read.csv("peabody_english/Peabody.percentiles.csv", header = TRUE)
...
for (participant in 1:nrow(participants_data)){
  ...
  dateOfBirth <- as.Date(participants_data[participant,"dateOfBirth"],
                format = "%d/%m/%Y")
