...
for (rawUUID in participants_uuids$V1)  {
    ...
    new_user_response$isError <- as.character(lapply(strsplit(new_user_response$tagValue1,"_"), 
    ...
    new_user_response$StimulusID <- as.character(lapply(strsplit(new_user_response$tagValue1,"_"), 
                                                        function(x) x[1]))
    new_user_response$Word <- lapply(new_user_response$Stimulus, 
                                       function(x) x[-1])
    new_user_response$Word <- as.character(lapply(new_user_response$Word, 
    ...
    new_user_response$isError <- ifelse(new_user_response$isError == "fout", 1, 0)
    
    user_responses <- rbind(user_responses, new_user_response)
    
    # Scoring explanation (coding is counterintuitive)
    #
    # The users task is to select stimuli that are spelling errors
    #
    # A tagValue1 that ends in "_fout" indicates that stimulus was a spelling 
    # error
    # A tagValue1 that ends in "_correct" indicates that stimulus was not a
    # spelling error
    ...
    # stimulus was a spelling error, but the user was incorrect 
    # (tagValue2 == "incorrect") because the stimulus was not a spelling error
    # (tagValue1 ends in "_not")
    
    TP <- length(which(str_sub(events$tagValue1, -4) == "fout" & 
                      events$tagValue2 == "correct"))
    FP <- length(which(str_sub(events$tagValue1, -7) == "correct" & 
                         events$tagValue2 == "incorrect"))
    TN <- length(which(str_sub(events$tagValue1, -7) == "correct" & 
                         events$tagValue2 == "correct"))
    FN <- length(which(str_sub(events$tagValue1, -4) == "fout" & 
                         events$tagValue2 == "incorrect"))
