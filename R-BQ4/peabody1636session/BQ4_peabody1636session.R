...
for (rawUUID in participants_uuids$V1){
  ...
  tagpairevents_1user <- subset(tagpairevents_data, userId == user & tagValue1 == "stimulusId")
  
  tagpairevents_1user$stimulus_nr <- lapply(strsplit(tagpairevents_1user$tagValue2,"_"), 
                                            function(x) x[5])
  ...
  start_date <- max(subset(screenviews_data, 
                           userId == user & 
                             screenName == "stimuliScreenBase13_13")[,c("viewDate")])
